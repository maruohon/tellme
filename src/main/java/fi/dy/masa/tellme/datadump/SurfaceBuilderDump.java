...
public class SurfaceBuilderDump
{
    public static List<String> getFormattedDump(Format format)
    {
        DataDump dump = new DataDump(1, format);

        for (Map.Entry<ResourceLocation, SurfaceBuilder<?>> entry : ForgeRegistries.SURFACE_BUILDERS.getEntries())
        {
            SurfaceBuilder<?> type = entry.getValue();
